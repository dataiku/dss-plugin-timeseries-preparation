...
def _make_resampling_params(groupby_cols=None):
    params = dku_timeseries.ResamplerParams(groupby_cols=groupby_cols)
    params.datetime_column = TIME_COL
    return params

def _make_resampler(groupby_cols=None):
    params = _make_resampling_params(groupby_cols)
...
def test_identity_resampling_group():
    ...
    resampler = _make_resampler(groupby_cols=GROUP_COL)
    df2 = resampler.transform(df, TIME_COL)
...
def test_half_freq_resampling_group():
    ...
    resampler = _make_resampler(GROUP_COL)
    df2 = resampler.transform(df, TIME_COL)
