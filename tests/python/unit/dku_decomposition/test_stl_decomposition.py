...
class TestSTLDecomposition:
    def test_STL_multiplicative(self, dku_config, input_df):
        ...
        assert np.mean(results["value1_trend"]) == 409265.35453951015
        assert np.mean(results["value1_seasonal"]) == 1.2698748679749625
        assert np.mean(results["value1_residuals"]) == 0.994103209790262
    ...
    def test_STL_additive(self, dku_config, input_df):
        ...
        np.testing.assert_equal(rounded_results, expected_array)
        assert np.mean(results["value1_trend"]) == 492101.01953512116
        assert np.mean(results["value1_seasonal"]) == 32625.652227975617
        assert np.mean(results["value1_residuals"]) == -5345.248686173712
    ...
    def test_advanced_smoothers(self, config, input_df, input_dataset_columns):
        ...
        result_df = decomposition.fit(df_prepared)
        assert result_df.shape == (26, 6)
        assert np.mean(result_df["value1_trend"]) == 476077.5935197391
        assert np.mean(result_df["value1_seasonal"]) == 43303.829557183984
        assert np.mean(result_df["value1_residuals"]) == -1.7910049511836126e-11  # -3.134258664571322e-11
        ...
        result_df = decomposition.fit(df_prepared)
        assert result_df.shape == (26, 6)
        assert np.mean(result_df["value1_trend"]) == 492101.01953512116
        assert np.mean(result_df["value1_seasonal"]) == 32625.652227975617
        assert np.mean(result_df["value1_residuals"]) == -5345.248686173712
    ...
    def test_advanced_degrees(self, config, input_df, input_dataset_columns):
        ...
        assert np.mean(result_df["value1_seasonal"]) == 40229.89887290868
